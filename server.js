...
const ADMIN_ROSTER = [
  {
    ...
    color: "#ff0000" // Unique identifier for UI if needed
  },
  {
...
async function syncAdminPrivileges() {
  ...
  try {
    ...
    for (const adminData of ADMIN_ROSTER) {
      ...
      const snapshot = await usersRef.where('displayName', '==', adminData.displayName).get();

      if (snapshot.empty) {
        console.log(`User "${adminData.displayName}" not found in database yet. They must log in first.`);
        continue;
      }

      // Update every instance of this name (usually there's only one)
      ...
      await batch.commit();
      console.log(`Successfully updated privileges for: ${adminData.displayName} (${adminData.rank})`);
    }
